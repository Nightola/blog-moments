/**
 * Nightola-227 FM 终极管理脚本 - 修正版
 * 确保观测站逻辑在模式切换时正确销毁与重建
 */
...
const qnaData = [
    ...
    {q: "为什么会喜欢夜晚？", a: "很久以前，感觉夜晚很浪漫，能看到很多星星。如果可以，没准和喜欢的人在夜色之下有段美好的回忆。以前总是在晚上偷偷用mp3听歌，新颖旋律冲击的感觉让我感觉夜晚很美好。只是现在好像对夜晚丧失了滤镜，但我依然喜欢自由支配、不被打扰的时光。", time: "2025-12-13 03:05"},
...
function clearObsStation() {
    if (obsState.channel) {
        obsState.channel.unsubscribe();
        obsState.channel = null;
    }
...
function render() {
    ...
    display.innerHTML = '';
    const filtered = (rawData.moments || []).filter(m => (currentYear === 'all' || m.year === currentYear) && (m.text||"").toLowerCase().includes(searchQuery.toLowerCase()));
...
// --- 观测站逻辑 (完全复刻你提供的 JS 判断逻辑) ---

function renderHome(container) {
    // 1. 还原 HTML
    container.innerHTML = `
        <section class="db-container"><header class="db-header"><h3 class="db-title">亚离解星观测站 <svg class="star-dec" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:14px;height:14px;display:inline;vertical-align:middle;animation:star-rot 8s linear infinite"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg></h3><span class="db-live-tag">LIVE</span></header><section class="db-grid"><article class="db-card status-box"><span class="db-label">当前状态</span><span class="status-wrap"><h4 id="status-text" class="status-val">同步中...</h4><span id="poke-bubble" class="db-bubble"></span></span><div class="status-footer"><span id="status-time" class="db-meta">连接卫星...</span><div class="poke-group"><span id="poke-count" class="poke-count">0 共鸣</span><button class="poke-btn" onclick="handlePoke(event)">戳我一下</button></div></div></article><article class="db-card time-box"><span class="db-label">本地时间</span><span class="time-wrap"><span id="local-clock" class="db-clock">00:00</span></span><span class="db-energy"><span class="energy-bar"><span id="energy-fill" class="energy-in" style="width:0%"></span></span><span id="energy-value" class="db-meta">--%</span></span></article><article class="db-card app-box full-row"><span class="db-label">正在使用 (PC)</span><span class="app-content"><svg class="app-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><path d="M8 21h8M12 17v4"></path></svg><span id="desktop-app-name" class="app-name">等待同步...</span></span><span id="desktop-dot" class="dot-indicator"></span></article><article class="db-card app-box full-row"><span class="db-label">正在使用 (MB)</span><span class="app-content"><svg class="app-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12" y2="18"></line></svg><span id="mobile-app-name" class="app-name">休息中</span></span><span id="mobile-dot" class="dot-indicator"></span></article><article class="db-card music-box full-row" id="music-card"><span class="db-label">正在聆听</span><div class="music-body"><img id="music-art" class="music-img" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="Art"><div class="music-content"><div class="music-info-wrap"><span id="music-track" class="music-title">目前没有在听歌...</span><span id="music-artist" class="music-sub">静音中</span></div></div></div></article><article class="db-card msg-box full-row"><div class="msg-header"><span class="db-label">私信互动</span><span class="db-meta">加密传输中</span></div><form action="https://formspree.io/f/mdkrvbap" method="POST" class="msg-form" id="contact-form"><div class="input-group"><textarea name="message" placeholder="此刻想对我说点什么？" required class="msg-input msg-area"></textarea><div id="reply-wrapper" style="display:none;margin-top:8px;"><input type="text" name="_replyto" id="reply-field" placeholder="如何称呼你或联系你？(选填)" class="msg-input reply-input"></div></div><div class="msg-ctrl"><button type="button" class="opt-btn" onclick="toggleReply(this)" id="opt-toggle">＋ 留下回信方式</button><button type="submit" class="msg-btn" id="submit-btn">发射信号</button></div></form></article></section><footer class="db-footer"><a href="https://nightola.mataroa.blog/blog/comments/" class="footer-link">评论区 &rarr;</a><span class="footer-note">Cloud Synced via Supabase</span></footer></section>
    `;
    ...
    const updateMusic = async () => {
        try {
            ...
            if(t && t['@attr'] && t['@attr'].nowplaying === 'true'){
                ...
                d.musicCard.classList.add('glowing-card');
            } else {
                ...
                d.musicCard.classList.remove('glowing-card');
            }
        } catch(e) {}
    };
    ...
    form.onsubmit = (e) => {
        ...
        btn.disabled = true; btn.textContent = '发射中...';
    };
}
